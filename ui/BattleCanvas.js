...
class BattleCanvas {
    ...
    init() {
        ...
        if (this.eventBus) {
            ...
            this.eventBus.on('combat:playerAction', this.onPlayerAction);
        }
    ...
    triggerDamage(targetId, damageType) {
        // damageType: 'physical' или 'magical'
        const color = damageType === 'physical' ? '#ff0000' : '#0066ff';
    ...
    triggerEvade(targetId) {
        this.animations.set(targetId, {
            ...
            timer: 8,
            data: { 
                offsetY: (Math.random() > 0.5 ? 1 : -1) * (10 + Math.random() * 7)
            }
        });
    }
    ...
    onPlayerAction(data) {
        // Обрабатываем действия игрока для анимаций
    ...
    updateBelt() {
        const beltSystem = this.game.beltSystem;
        if (beltSystem) {
            this.beltSlots = beltSystem.getBeltInfo().slots;
        }
    }
    ...
    async drawEntityWithAnimations(entity, x, y) {
        ...
        if (anim && anim.type === 'damage') {
            ...
            this.ctx.globalAlpha = 0.3 + (anim.timer / 10) * 0.3;
            this.ctx.fillRect(drawX, drawY, this.spriteCellSize, this.spriteCellSize);
    ...
    drawBottomPanel() {
        ...
        for (let row = 0; row < this.abilityRows; row++) {
            for (let col = 0; col < this.abilityCols; col++) {
                const x = col * this.abilityCellSize;
                const cellY = y + (row * this.abilityCellSize);
                
                this.ctx.strokeRect(x, cellY, this.abilityCellSize, this.abilityCellSize);
                
                // Ячейки способностей (первые 12 колонок)
                const abilityIndex = row * 12 + col;
                if (col < 12 && this.abilities[abilityIndex]) {
                ...
                // Ячейки пояса (последние 6 ячеек в 3-й строке)
                if (row === 2 && col >= 12) {
                    const beltIndex = col - 12;
                    if (this.beltSlots[beltIndex]?.item) {
                        const item = this.beltSlots[beltIndex].item;
                        this.ctx.fillStyle = '#ffaa44';
                        this.ctx.fillRect(x + 5, cellY + 5, 40, 40);
                        ...
                        if (item.count > 1) {
                            ...
                            this.ctx.fillText(item.count, x + 30, cellY + 40);
                        }
                    }
                }
            }
        ...
        // Область портрета (первые 2 колонки, 2 строки)
        this.ctx.fillStyle = '#2a2a2a';
        this.ctx.fillRect(0, y, 100, 100);
        this.ctx.strokeStyle = '#666';
        this.ctx.strokeRect(0, y, 100, 100);
    }
        ...
        if (clickedEntity.isEnemy) {
            if (this.selectedAbility) {
                ...
                this.eventBus?.emit('combat:playerAction', {
                    player: this.game.player,
                    actionType: 'attack',
                    data: { targetId: clickedEntity.id }
                });
            }
        }
    ...
    handleBottomClick(offsetX, offsetY) {
        ...
        if (col < 0 || col >= this.abilityCols || row < 0 || row >= this.abilityRows) return;
        
        const index = row * this.abilityCols + col;
        
        // Ячейки способностей (первые 12 колонок в каждой строке)
        if (col < 12) {
            const abilityIndex = row * 12 + col;
        ...
        // Пояс (последние 6 ячеек в 3-й строке)
        if (row === 2 && col >= 12) {
            const beltIndex = col - 12;
            if (this.beltSlots[beltIndex]?.item) {
                this.eventBus?.emit('combat:playerAction', {
    ...
    destroy() {
        if (this.eventBus) {
            ...
            this.eventBus.off('combat:playerAction', this.onPlayerAction);
        }
