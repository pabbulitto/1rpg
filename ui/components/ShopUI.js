...
class ShopUI {
    ...
    /**
     * Отрисовать список предметов для продажи
     * @returns {string} HTML
     */
    renderSellableItems() {
        const items = this.getInventoryItems(); // получает сырые ItemData
        if (!items || items.length === 0) {
        ...
        items.forEach((item, index) => {
            ...
            // item — это ItemData, поля доступны напрямую
    ...
    bindEvents() {
        ...
        if (this._clickHandler) {
            this.modalContainer.removeEventListener('click', this._clickHandler);
        }
        ...
        this._clickHandler = (e) => {
            ...
            if (target.classList.contains('buy-btn')) {
                ...
                if (!itemId || !this.onBuyItem) return;
            ...
            // Продажа - ВАЖНО: убрать refreshShopData() отсюда
        ...
        this.modalContainer.addEventListener('click', this._clickHandler);
    }

    refreshShopData() {
        // Только обновляем данные, но НЕ перерендериваем
        this.currentShop = this.getShopInfo ? this.getShopInfo() : this.currentShop;
        // Обновим только цифру золота и кнопки
        const goldElement = this.modalContainer.querySelector('.gold-text');
        if (goldElement) {
            goldElement.textContent = this.currentShop.playerGold;
        }
    }
