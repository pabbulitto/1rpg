...
class CharacterCreationUI {
    ...
    constructor({ container, dataService, onComplete }) {
        ...
        // Состояние
        this.selectedRace = 'northerner'; // ID расы по умолчанию
        this.selectedClass = 'strongman';  // ID класса по умолчанию
        this.spentPoints = {
            strength: 0,
            dexterity: 0,
            constitution: 0,
    ...
    // Рендер HTML
    render() {
        const raceOptions = Object.entries(this.races).map(([id, race]) => 
            `<option value="${id}" ${id === this.selectedRace ? 'selected' : ''}>${race.name}</option>`
        ).join('');
        
        const classOptions = Object.entries(this.classes).map(([id, cls]) => 
