...
class InventoryUI {
    ...
    createInfoModal(item, x, y, itemIndex) {
        ...
        const isCorpse = item.type === 'corpse';
        
        // Для трупа не показываем stats, price и другие детали
        if (isCorpse) {
            modal.innerHTML = `
                <div class="modal-header">
                    <h4>${item.name}</h4>
                    <div class="item-type">труп</div>
                </div>
                <div class="modal-desc">Внутри могут быть ценные вещи. Осмотрите, бросив на землю.</div>
            ...
            // Обычная модалка для других предметов
