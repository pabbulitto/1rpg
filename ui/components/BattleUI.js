...
class BattleUI {
    ...
    lootAllCorpse(corpseId) {
        ...
        // Если после лута у трупа не осталось предметов - удаляем труп
        if (corpse.container.getItemCount() === 0) {
    ...
    lootCorpseItem(corpseId, itemIndex) {
        ...
        // Если после лута у трупа не осталось предметов - удаляем труп
        if (corpse.container.getItemCount() === 0 && 
            Object.values(corpse.container.getAllEquipment()).filter(Boolean).length === 0) {
            this.game.zoneManager.removeEntity(corpseId);
        }
