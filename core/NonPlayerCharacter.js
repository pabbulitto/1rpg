...
class NonPlayerCharacter extends Character {
    ...
    loadFromConfig(config, level = 1, enemyType = null) {
        ...
        if (config.startingEquipment && this.equipmentService) {
            Object.entries(config.startingEquipment).forEach(([slot, itemId]) => {
                ...
                if (item) {
                    ...
                    console.warn(`NonPlayerCharacter: не удалось создать предмет ${itemId} для экипировки`);
                }
            });
        }
        ...
        // Награды
        this.expReward = config.experienceReward || config.baseExp || 10;
        this.goldReward = config.goldReward || config.baseGold || 5;
        this.manualLoot = config.manualLoot || [];
        ...
        // Труп и респавн
        this.corpseDecay = config.corpseDecay || 300;
