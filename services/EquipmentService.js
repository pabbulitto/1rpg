...
class EquipmentService {
    constructor(eventBus, statManager) {
    ...
    canEquip(item, currentEquipment, player = null) {
        ...
        // TODO: будущие проверки (уровень, класс, атрибуты)
    ...
    /**
     * Проверить требования предмета (для будущего расширения)
     */
    checkRequirements(requirements, player) {
        // TODO: реализовать проверку уровня, класса, атрибутов
    ...
    applyEquipmentModifiers(slot, item) {
        ...
        // 1. Удаляем старый модификатор
        this.statManager.removeModifier(sourceId);
        // 2. Добавляем новый модификатор (если предмет есть)
        if (item && item.stats && Object.keys(item.stats).length > 0) {
            this.statManager.addModifier(sourceId, item.stats);
        }
        // Вариант A: Через window.game (если доступен)
        let playerData = null;
        if (window.game && window.game.gameState) {
            playerData = window.game.gameState.getPlayer();
        }
        // Вариант B: Через EventBus - просим GameState отправить данные
        this.eventBus.emit('player:statsChanged', playerData);
        // 4. Дополнительно: явно обновляем инвентарь UI
