  ...
  async init() {
    try {
      ...
      if (!hasExistingSave) {
        ...
        const potion = itemFactory.create('health_potion', 3);
        if (potion) this.gameState.playerContainer.addItem(potion);
        const sword = itemFactory.create('rusty_sword', 1);
        if (sword) this.gameState.playerContainer.addItem(sword);
