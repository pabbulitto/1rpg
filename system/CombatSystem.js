...
// === ЗАГОТОВКА ПОД СИСТЕМУ ИНИЦИАТИВЫ ===
    /**
     * Добавить действие в очередь
     * @param {Object} action - действие {type, data, actor}
     */
    queueAction(action) {
        this.actionQueue.push(action);
        console.log('CombatSystem: действие добавлено в очередь', action);
    }

    /**
     * Очистить очередь (вернуть все действия)
     * @returns {Array}
     */
    clearActionQueue() {
    ...
    /**
     * Получить следующее действие из очереди
     * @returns {Object|null}
     */
    getNextAction() {
    ...
    executeEnemyAttack(enemy) {
        ...
        const attackResult = enemy.attackPlayer(playerStats);  // ← теперь объект
        
        if (attackResult && attackResult.damage > 0 && player.takeDamage) {
            const result = player.takeDamage(attackResult.damage, {
                isCritical: attackResult.isCritical
            });
            
            // Логи для UI
            this.eventBus.emit('log:add', {
                message: `${enemy.name} наносит вам ${attackResult.damage} урона${attackResult.isCritical ? ' (КРИТ!)' : ''}!`,
                type: 'battle'
            });
        ...
        return attackResult ? attackResult.damage : 0;
    }
    /**
     * Враг побежден
