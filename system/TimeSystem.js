class TimeSystem {
    constructor(gameState) {
        ...
        this.currentTick = 0;
        this.tickInterval = 1000; // 1 секунда = 1 игровой тик
    ...
    processTick() {
        ...
        // Обновляем игровое время (1 тик = 10 минут игрового времени)
        this.updateGameTime();
    ...
    updateGameTime() {
        // 1 тик = 10 минут игрового времени
    ...
    registerExistingSystems() {
        ...
        if (gameState && gameState.conditions) {
            this.registerCondition(() => {
                // Обновление условий (голод, жажда) со временем
                // TODO: реализовать логику ухудшения условий
            });
        }
    }
    ...
    emitTimeEvent(eventType, data) {
        ...
        if (window.game?.uiManager) {
            window.game.uiManager.addToLog(
                `[Время: ${this.formatTime()}] ${eventType}`,
                'time'
            );
        }
