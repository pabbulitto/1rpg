class TimeSystem {
    constructor(gameState) {
        ...
        this.temporarySubscribers = new Map();    
    ...
    processTick() {
        ...
        // 4. Постоянные кастомные подписчики
        for (const callback of this.persistentSubscribers.values()) {
            if (typeof callback === 'function') {
                callback(this.currentTick, this.gameTime);
            }
        }
        
        // 5. Временные кастомные подписчики
        for (const callback of this.temporarySubscribers.values()) {
            if (typeof callback === 'function') {
                callback(this.currentTick, this.gameTime);
            }
        }
    ...
    registerCustom(id, callback, persistent = false) {
        if (persistent) {
            this.persistentSubscribers.set(id, callback);
        } else {
            this.temporarySubscribers.set(id, callback);
        }
        return () => this.unregisterCustom(id);
    }

    unregisterCustom(id) {
        this.persistentSubscribers.delete(id);
        this.temporarySubscribers.delete(id);
    }
