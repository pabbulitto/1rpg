...
class ShopSystem {
  ...
  _executeBuy(itemId, player, validationResult) {
      ...
      if (!item) {
          // Возвращаем деньги
          this.gameState.addGold(itemData.price);
      ...
      // ИСПРАВЛЕНО: добавляем в контейнер напрямую
  ...
  sellItem(itemIndex, player) {
      ...
      // ===== ИСПРАВЛЕНО: работа со стаками =====
      if (item.stackable && item.count > 1) {
          // Продаем 1 единицу из стека
          item.count--;
          
          // !!! ЕДИНСТВЕННАЯ ДОБАВЛЕННАЯ СТРОКА !!!
          this.gameState.playerContainer._markDirty();
          
          const goldReceived = sellPrice;
          this.gameState.addGold(goldReceived);
          ...
          return { 
              success: true, 
              message: `Продано: ${item.name} за ${goldReceived} золота`,
              goldReceived: goldReceived,
              item: item.getInfo()
          };
      } else {
          // Продаем весь предмет
  ...
  /**
   * Получить информацию о магазине для UI
   * @returns {Object|null} информация о магазине
   */
  getShopInfo() {
