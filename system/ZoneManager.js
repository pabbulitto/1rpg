class ZoneManager {
  ...
  getRandomEnemyFromRoom() {
    const roomInfo = this.getCurrentRoomInfo();
    if (!roomInfo || !roomInfo.enemies || roomInfo.enemies.length === 0) {
      return null;
    }
    
    const randomIndex = Math.floor(Math.random() * roomInfo.enemies.length);
    return roomInfo.enemies[randomIndex];
  }
